...
#        self.petsc_jacobian_4d = PETSc_MHD_NL_Jacobian_Matrix.PETScJacobian(self.da1, self.da5, nx, ny, self.ht, self.hx, self.hy)
        ...
        OptDB.setValue('snes_max_it', 5)
        OptDB.setValue('snes_convergence_test', 'skip')
        OptDB.setValue('snes_lag_jacobian', 5)
        ...
        OptDB.setValue('snes_convergence_test', 'default')
        OptDB.setValue('snes_lag_jacobian', 1)
        
        self.snes_corr = PETSc.SNES().create()
        self.snes_corr.setFunction(self.petsc_function.snes_mult, self.f)
        self.snes_corr.setJacobian(self.updateJacobian, self.J)
        self.snes_corr.setFromOptions()
        self.snes_corr.getKSP().setType('preonly')
        self.snes_corr.getKSP().getPC().setType('lu')        
        self.snes_corr.getKSP().getPC().setFactorSolverPackage('mumps')
...
#            del self.hdf5_viewer
    ...
    def run(self):
        ...
        for itime in range(1, self.nt+1):
            ...
            self.snes_corr.solve(None, self.x)
            ...
            if PETSc.COMM_WORLD.getRank() == 0:
                ...
                print("  Corrector Solver:  %5i iterations,   funcnorm = %24.16E" % (self.snes_corr.getIterationNumber(), self.snes_corr.getFunctionNorm()) )
