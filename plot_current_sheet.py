...
#        plt.subplots_adjust(hspace=0.2, wspace=0.25)
        ...
        self.axes["A"]     = plt.subplot(gs[ 8:12,  0])
        self.axes["PB"]    = plt.subplot(gs[12:16,  0])
        self.axes["P"]     = plt.subplot(gs[16:20,  0])
        self.axes["Vx"]    = plt.subplot(gs[ 0:10,1:3])
        self.axes["Vy"]    = plt.subplot(gs[10:20,1:3])
        self.axes["Babs"]  = plt.subplot(gs[ 0:10,3:5])
        self.axes["Emag"]  = plt.subplot(gs[10:12,3:5])
        self.axes["Evel"]  = plt.subplot(gs[12:14,3:5])
        self.axes["E"]     = plt.subplot(gs[14:17,3:5])
        self.axes["H"]     = plt.subplot(gs[17:20,3:5])
        ...
        self.axes["PB"].set_title('$P_B (x,y)$')
...
#         self.cbars["Vy"] = self.figure.colorbar(self.conts["Vy"], ax=self.axes["Vy"], orientation='vertical', ticks=self.VyTicks)
        ...
        self.conts["A" ] = self.axes["A"].contourf(self.x, self.y, self.A.T, self.ATicks, cmap=cm.jet, extend='both')
        self.cbars["A" ] = self.figure.colorbar(self.conts["A"], ax=self.axes["A"], orientation='vertical', ticks=self.ATicks)
        
        self.conts["PB"] = self.axes["PB"].contourf(self.x, self.y, self.PB.T, self.PTicks, cmap=cm.jet, extend='both')
        self.cbars["PB"] = self.figure.colorbar(self.conts["PB"], ax=self.axes["PB"], orientation='vertical', ticks=self.PTicks)
        ...
        self.conts["Babs"] = self.axes["Babs"].contour(self.x, self.y, self.A.T, self.ATicks, cmap=cm.jet, extend='neither')
    ...
    def update_boundaries(self):
        ...
        dB = 0.1 * (Bmax - Bmin)
        self.Bnorm = colors.Normalize(vmin=Bmin-dB, vmax=Bmax+dB)
        ...
        dV = 0.1 * (Vmax - Vmin)
        self.Vnorm = colors.Normalize(vmin=Vmin-dV, vmax=Vmax+dV)
...
#        self.cbars["Babs"] = self.figure.colorbar(self.conts["Babs"], ax=self.axes["Babs"], orientation='vertical', ticks=self.divVTicks)
        ...
        self.conts["Babs"] = self.axes["Babs"].contour(self.x, self.y, self.A.T, self.ATicks, extend='neither')
