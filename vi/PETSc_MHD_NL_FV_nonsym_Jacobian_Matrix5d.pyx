...
#            print("     Matrix")
    ...
    cdef np.float64_t ux_dx_fx(self, np.ndarray[np.float64_t, ndim=2] A,
        ...
        # (i-2, j  )
        A[0,2] += (F[i,   j  ] - F[i-2, j  ]) * sign * self.fac_dx
        ...
        # (i,   j-2)
        A[2,0] += (F[i+1, j-1] - F[i-1, j-1]) * sign * self.fac_dx
        ...
        # (i,   j+2)
        A[2,4] += (F[i+1, j+1] - F[i-1, j+1]) * sign * self.fac_dx
        ...
        # (i+2, j  )
        A[4,2] += (F[i+2, j  ] - F[i,   j  ]) * sign * self.fac_dx
    ...
    cdef np.float64_t ux_dx_fy(self, np.ndarray[np.float64_t, ndim=2] A,
        ...
        # (i-2, j  )
        A[0,2] += 2 * (F[i,   j  ] - F[i-2, j  ]) * sign * self.fac_dx
        
        # (i-2, j+1)
        A[0,3] += 2. * (F[i,   j  ] - F[i-2, j  ]) * sign * self.fac_dx
        
        
        # (i-1, j-1)
        A[1,1] += 2. * (F[i+1, j-1] - F[i-1, j-1]) * sign * self.fac_dx
        ...
        # (i-1, j+2)
        A[1,4] += 2. * (F[i+1, j+1] - F[i-1, j+1]) * sign * self.fac_dx
        
        
        # (i,   j-1)
        A[2,1] += 2. * (F[i+1, j-1] - F[i-1, j-1]) * sign * self.fac_dx
        ...
        # (i,   j+2)
        A[2,4] += 2. * (F[i+1, j+1] - F[i-1, j+1]) * sign * self.fac_dx
        
        
        # (i+1, j  )
        A[3,2] += 2. * (F[i+2, j  ] - F[i,   j  ]) * sign * self.fac_dx
        
        # (i+1, j+1)
        A[3,3] += 2. * (F[i+2, j  ] - F[i,   j  ]) * sign * self.fac_dx
    ...
    cdef np.float64_t uy_dy_fx(self, np.ndarray[np.float64_t, ndim=2] A,
        ...
        # (i-1, j-1)
        A[1,1] += 2. * (F[i-1, j+1] - F[i-1, j-1]) * sign * self.fac_dy
        
        # (i-1, j  )
        A[1,2] += 2. * (F[i-1, j+1] - F[i-1, j-1]) * sign * self.fac_dy
        
        
        # (i,   j-2)
        A[2,0] += 2. * (F[i,   j  ] - F[i,   j-2]) * sign * self.fac_dy
        ...
        # (i,   j+1)
        A[2,3] += 2. * (F[i,   j+2] - F[i,   j  ]) * sign * self.fac_dy
        
        
        # (i+1, j-2)
        A[3,0] += 2. * (F[i,   j  ] - F[i,   j-2]) * sign * self.fac_dy
        ...
        # (i+1, j+1)
        A[3,3] += 2. * (F[i,   j+2] - F[i,   j  ]) * sign * self.fac_dy
        
        
        # (i+2, j-1)
        A[4,1] += 2. * (F[i+1, j+1] - F[i+1, j-1]) * sign * self.fac_dy

        # (i+2, j  )
        A[4,2] += 2. * (F[i+1, j+1] - F[i+1, j-1]) * sign * self.fac_dy
    ...
    cdef np.float64_t uy_dy_fy(self, np.ndarray[np.float64_t, ndim=2] A,
        ...
        # (i-2, j  )
        A[0,2] += (F[i-1, j+1] - F[i-1, j-1]) * sign * self.fac_dy
        ...
        # (i,   j-2)
        A[2,0] += (F[i,   j  ] - F[i,   j-2]) * sign * self.fac_dy
        ...
        # (i,   j+2)
        A[2,4] += (F[i,   j+2] - F[i,   j  ]) * sign * self.fac_dy
        ...
        # (i+2, j  )
        A[4,2] += (F[i+1, j+1] - F[i+1, j-1]) * sign * self.fac_dy
